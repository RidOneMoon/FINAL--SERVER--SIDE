...
const getIssues = async (req, res) => {
  ...
  if (currentUser.role === "staff") {
    filter.assignedStaffId = new ObjectId(userId);
  }
  ...
  try {
    ...
    const totalIssues = await IssueService.getTotalIssues(userId);
...
const getSingleIssue = async (req, res) => {
  ...
  // console.log("get single issue", req.params);
...
const createIssue = async (req, res) => {
  ...
  try {
    ...
    if (subscriptionStatus === 'free' && totalIssues >= 3) {
    ...
    const userId = currentUser.userId;
    const result = await IssueService.createIssue(body, userId);
...
const editOwnIssue = async (req, res) => {
  ...
  const updatePayload = body.updatedData;
...
const getResolvedIssues = async(req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "An internal server error occurred while fetching resolved issues.",
    });
  }
}
